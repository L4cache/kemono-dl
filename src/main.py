...
class downloader:
    ...
    def get_inline_images(self, post, content_soup):
        ...
        for index, inline_image in enumerate(inline_images):
            ...
            _fileserver = 'file.' if 'pawchive' in post['post_variables']['site'] else ''
            file['file_variables'] = {
                'filename': filename,
                'ext': file_extension[1:],
                'url': f"https://{_fileserver}{post['post_variables']['site']}/data{inline_image['src']}",
                'hash': file_hash,
    ...
    def clean_post(self, post:dict, user:dict, domain:str, post_jr:dict = None):
        ...
        if self.attachments:
            ...
            for index, attachment in enumerate(post['attachments']):
                ...
                file_hash = m.group(0) if m else None
                _fileserver = 'file.' if 'pawchive' in domain else ''
                file['file_variables'] = {
                    'filename': filename,
                    'ext': file_extension[1:],
